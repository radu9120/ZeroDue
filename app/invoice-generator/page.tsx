...
export default function InvoiceGeneratorPage() {
  ...
  // Check if user just signed up/in and has pending invoice data
  useEffect(() => {
    const checkPendingInvoice = async () => {
      ...
      if (user) {
        const pendingData = sessionStorage.getItem("pending_invoice");
        if (pendingData) {
          try {
            const invoiceData = JSON.parse(pendingData);
            // User is authenticated, create the invoice now
            ...
            if (response.ok) {
              ...
              // Redirect to dashboard to see the invoice
              window.location.href = `/dashboard/invoices?new_invoice=${data.invoice_id}`;
            }
          } catch (error) {
  ...
  const handleCreateInvoice = () => {
    ...
    // Store invoice data in sessionStorage so it persists after sign-up/sign-in
    const invoiceData = {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-slate-600 dark:text-slate-400 mb-6">
                Create a free account to save your invoice and access it
                anytime. You can also track payments and send reminders.
              </p>
              <div className="space-y-4">
                {/* Sign In Link */}
                <div className="text-center mb-4">
                  <p className="text-sm text-slate-600 dark:text-slate-400">
                    ...
                    <Link
                      href={`/sign-in?redirect_url=/invoice-generator`}
                ...
                {/* Sign Up with Google */}
                <Link href={`/sign-up?redirect_url=/invoice-generator`}>
                  <Button
                    className="w-full bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-2 border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600"
                    size="lg"
                  >
